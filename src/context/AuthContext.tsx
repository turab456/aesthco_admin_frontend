...
const ACCESS_COOKIE = "admin_auth_token";
...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const signIn = useCallback(
    async (payload: LoginPayload) => {
      try {
        const data = await loginSuperAdmin(payload);
        persistTokens(
          data.admin_auth_token || data.accessToken,
          data.admin_refresh_token || data.refreshToken
        );
        setUser(data.user);
      } catch (error) {
        const message =
  ...
  const signOut = useCallback(async () => {
    ...
    try {
      await logoutSuperAdmin(refreshToken ?? undefined);
    } catch (error) {
